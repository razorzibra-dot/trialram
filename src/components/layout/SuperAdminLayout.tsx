      ...
      <div className="flex h-16 items-center px-6 border-b border-neutral-200">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            <span className="text-lg font-semibold text-neutral-900 tracking-tight">Super Admin</span>
            <span className="text-xs text-neutral-500 font-medium">Platform Control</span>
          </div>
        </div>
      </div>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center space-x-3 px-3 py-2 text-sm font-medium rounded-md transition-all duration-200 group",
                      item.current
                  ...
                  >
                    <Icon className={cn(
                      ...
                      item.current ? "text-accent-500" : "text-neutral-400 group-hover:text-neutral-500"
                    )} />
            ...
            <div className="flex h-14 sm:h-16 items-center justify-between px-3 sm:px-4 lg:px-6">
              ...
              <div className="flex items-center space-x-2 sm:space-x-4 flex-1 min-w-0">
                ...
                <nav className="hidden sm:flex items-center space-x-1 sm:space-x-2 text-sm min-w-0 flex-1">
                  {breadcrumbs.map((crumb, index) => (
                    <React.Fragment key={crumb.name}>
                      {index > 0 && <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" style={{ color: '#9EAAB7' }} />}
                      {crumb.href ? (
                        <button
                          onClick={() => navigate(crumb.href!)}
                          className="font-medium transition-colors hover:bg-neutral-100 px-1 sm:px-2 py-1 rounded-md truncate"
                          style={{ color: '#7A8691' }}
                        ...
                        <span className="font-semibold px-1 sm:px-2 py-1 rounded-md truncate" style={{
                          color: '#2C3E50',
                ...
                <div className="sm:hidden flex-1 min-w-0">
                  <h1 className="text-lg font-semibold text-neutral-900 truncate flex items-center gap-2">
                    <Crown className="h-4 w-4 text-accent-500 flex-shrink-0" />
              ...
              <div className="flex items-center space-x-1 sm:space-x-3">
                ...
                <Button
                  ...
                  className="relative touch-target hover:bg-neutral-100 p-2"
                  style={{ color: '#2C3E50' }}
                >
                  <RefreshCw className={cn("h-5 w-5", isRefreshing && "animate-spin")} />
                </Button>

                <Button variant="ghost" size="sm" className="relative touch-target hover:bg-neutral-100 p-2" style={{ color: '#2C3E50' }}>
                  <Bell className="h-5 w-5" />
                  {systemHealth?.alerts.filter(a => !a.resolved).length > 0 && (
                    <Badge className="absolute -top-1 -right-1 h-4 w-4 sm:h-5 sm:w-5 p-0 text-xs animate-pulse" style={{
                      backgroundColor: '#E74C3C',
                      color: 'white'
                    }}>
                      {systemHealth.alerts.filter(a => !a.resolved).length > 9 ? '9+' : systemHealth.alerts.filter(a => !a.resolved).length}
                    </Badge>
                  )}
                </Button>
