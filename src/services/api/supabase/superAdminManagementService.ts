...
export const supabaseAdminManagementService: ISuperAdminManagementService = {
  ...
  async getActionLogs(superAdminId?: string, limit = 100): Promise<SuperAdminActionLog[]> {
    try {
      ...
      if (superAdminId) {
        query = query.eq('user_id', superAdminId);
      }
  ...
  async getAllTenantAccesses(limit = 1000): Promise<SuperAdminTenantAccess[]> {
    try {
      ...
      console.error('❌ Failed to get all tenant accesses:', error);
      return [];
    }
  }
};
 ...
 */
function mapUserToSuperAdminDTO(user: any): SuperAdminDTO {
  return {
    id: user.id,
    email: user.email,
    name: user.name,
 ...
 */
function mapAccessToDTO(access: any): SuperAdminTenantAccess {
  return {
    id: access.id,
