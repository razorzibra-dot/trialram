...
import { customerService, productService, multiTenantService } from '../serviceFactory';

class MockJobWorkService {
  ...
  async calculatePrice(productId: string, pieces: number, size: string): Promise<JobWorkPriceCalculation> {
    ...
    try {
      // Get actual product data for base price
      const product = await productService.getProduct(productId);
      const basePrice = product.price || 1000;

      // Size multipliers for job work complexity
      ...
      const totalPrice = calculatedPrice * pieces;
      ...
      // Fallback to default pricing if product not found
  ...
  private async getCustomerShortName(customerId: string): Promise<{ name: string; short_name: string; }> {
    try {
      // Get actual customer data from unified Customer service
      const customer = await customerService.getCustomer(customerId);

      // Generate short name from company name
      const shortName = this.generateCustomerShortName(customer.company_name);
      ...
      return { name: 'Unknown Customer', short_name: 'UNK' };
    }
  }

  private generateCustomerShortName(companyName: string): string {
    // Generate 3-4 character short name from company name
    ...
    if (words.length === 1) {
      ...
      // Multiple words: take first char of first 4 words
...
export const mockJobWorkService = new MockJobWorkService();