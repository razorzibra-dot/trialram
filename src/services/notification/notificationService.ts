...
import type {
  Notification,
  UINotificationPreferences as NotificationPreferences,
  NotificationFilters,
...
export type { Notification, NotificationPreferences, NotificationFilters };
...
class MockNotificationService {
  ...
  async markAsRead(id: string): Promise<Notification> {
    try {
      ...
      if (error instanceof Error && 'code' in error) {
        throw error; // Already a ServiceError
      }
  ...
  notify(options: {
    ...
    try {
      ...
      // For notify, we don't throw as it's a fire-and-forget operation
