...
export const mockServiceContractService = {
  ...
  async getServiceContracts(filters: ServiceContractFilters = {}): Promise<PaginatedResponse<ServiceContractType>> {
    try {
      ...
      filtered.sort((a, b) => {
        const aVal = (a as ServiceContractType & Record<string, unknown>)[sortBy as keyof ServiceContractType];
        const bVal = (b as ServiceContractType & Record<string, unknown>)[sortBy as keyof ServiceContractType];
  ...
  async createServiceContract(data: ServiceContractCreateInput): Promise<ServiceContractType> {
    ...
    const newContract: ServiceContractType = {
      ...
      serviceType: data.serviceType as ServiceContractType['serviceType'],
      status: 'draft',
      priority: (data.priority as ServiceContractType['priority']) || 'medium',
      value: data.value,
      currency: data.currency || 'USD',
      billingFrequency: data.billingFrequency as ServiceContractType['billingFrequency'],
  ...
  async addServiceContractDocument(data: ServiceContractDocumentCreateInput): Promise<ServiceContractDocumentType> {
    const doc: ServiceContractDocumentType = {
      ...
      fileType: data.fileType as ServiceContractDocumentType['fileType'],
      fileSize: data.fileSize,
      filePath: data.filePath,
      documentType: data.documentType as ServiceContractDocumentType['documentType'],
  ...
  async addServiceContractIssue(data: ServiceContractIssueCreateInput): Promise<ServiceContractIssueType> {
    const issue: ServiceContractIssueType = {
      ...
      severity: data.severity as ServiceContractIssueType['severity'],
      category: data.category as ServiceContractIssueType['category'],
      status: 'open',
      assignedToUserId: data.assignedToUserId,
