...
COMMENT ON POLICY "users_view_tenant_contract_terms" ON contract_terms IS 'Users can only view contract terms in their tenant';-- Create sales pipeline tables: opportunities, deals, sales_activities
...
GRANT SELECT ON sales_activities_with_details TO authenticated;-- Add product hierarchy columns to products table
...
-- (No changes needed as RLS is already tenant-based)-- Add payment processing and revenue recognition columns to deals table
...
WHERE cp.id IS NULL
ON CONFLICT DO NOTHING;-- Create opportunities table for sales pipeline management
...
-- Sample data inserts removed to avoid FK constraint violations with non-existent customer/product records-- Create product_categories table for hierarchical product categorization
...
COMMENT ON COLUMN sales.created_by IS 'User who created this deal';-- Migration: Add deal_type, contract_id and extend deal_items for PRODUCT/SERVICE support
-- Date: 2025-12-04
